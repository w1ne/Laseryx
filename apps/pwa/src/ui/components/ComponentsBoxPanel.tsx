...
export function renderEnclosureWorkspace(document: Document, workspace: EnclosureWorkspace): Document {
  ...
  if (!workspace.enclosure.result) {
    ...
    if (members.length > 1) groups.push({ id: `components-box:panel:${workspace.sourcePanel.id}`, name: workspace.sourcePanel.name, memberIds: members });
  } else {
...
export function ComponentsBoxPanel({ document, onDocumentChange }: ComponentsBoxPanelProps) {
  ...
  const savePanel = (width: number, height: number) => {
    ...
    }); setEditor(null); setMessage("Panel ready · 160 × 100 mm");
  };
  ...
  const makeBox = (settings: BoxSettings) => {
    if (!workspace) return;
    const sourcePanel = { ...workspace.sourcePanel, height: Math.hypot(settings.depth, settings.rearHeight - settings.frontHeight) };
    const candidate = { ...workspace, sourcePanel, enclosure: { ...workspace.enclosure, parameters: { frontHeight: settings.frontHeight, rearHeight: settings.rearHeight, thickness: settings.thickness, clearance: settings.clearance, fingerTarget: settings.fingerTarget } }, coupon: { ...workspace.coupon, confirmed: !settings.includeCoupon }, sheetLayout: undefined };
  ...
  return <section className="components-box">
    ...
    {open && <div className="components-box__body">
      ...
      {editor === "box" && <BoxDialog onConfirm={makeBox} onCancel={() => setEditor(null)} />}
...
function PanelForm({ initial, onSave, onCancel }: { initial?: PanelDesign; onSave: (width: number, height: number) => void; onCancel: () => void }) {
  ...
  </div><div className="components-box__editor-actions"><button type="button" onClick={() => onSave(width, height)}>Save panel</button><button type="button" onClick={onCancel}>Cancel</button></div></div>;
}
