...
function InstanceEditor({ panel, instance, onSave, onDelete, onCancel }: { panel: PanelDesign; instance: ComponentInstance; onSave: (changes: Partial<Omit<ComponentInstance, "id" | "kind">>) => boolean; onDelete: () => void; onCancel: () => void }) {
  ...
  const [bodyDepth, setBodyDepth] = useState(instance.mechanics?.body?.depth?.toString() ?? ""), [missing, setMissing] = useState(instance.mechanics?.missing?.join(", ") ?? ""), [holes, setHoles] = useState(instance.mechanics?.mountingHoles?.map(({ x, y, diameter }) => `${x},${y},${diameter}`).join(";") ?? "");
  const parsedHoles = holes.trim() ? holes.split(";").map((row) => { const [holeX, holeY, diameter] = row.split(",").map(Number); return { x: holeX, y: holeY, diameter }; }) : [];
  const mechanics = instance.mechanics ? { ...instance.mechanics, ...(instance.mechanics.body ? { body: { ...instance.mechanics.body, ...(bodyDepth === "" ? { depth: undefined } : { depth: Number(bodyDepth) }) } } : {}), mountingHoles: parsedHoles, missing: missing.split(",").map((item) => item.trim()).filter(Boolean) } : undefined;
  const changes = { dimensions, transform: { ...instance.transform, e: x, f: y }, mechanics } as Partial<Omit<ComponentInstance, "id" | "kind">>;
  let error = "";
  try { error = [...validatePanel(updateComponentInstance(panel, instance.id, changes)).map(({ message }) => message), ...validateMechanics(mechanics)].join(" "); } catch (cause) { error = cause instanceof Error ? cause.message : "Component values are invalid."; }
  ...
  return <div role="dialog" aria-label={`Edit ${instance.name}`} className="components-box__editor"><strong>{instance.name}</strong>
    ...
    {instance.mechanics && <details><summary>Mechanical details</summary><label>Body depth<input aria-label="Instance body depth" type="number" min="0.01" value={bodyDepth} onChange={(event) => setBodyDepth(event.target.value)} /></label><label>Mounting holes<input aria-label="Instance mounting holes" value={holes} onChange={(event) => setHoles(event.target.value)} /></label><label>Missing measurements<input aria-label="Instance missing measurements" value={missing} onChange={(event) => setMissing(event.target.value)} /></label></details>}
